...
| Seguridad e IAM | `roles`, `usuarios`, `grupo_menu`, `menu`, `menu_rol` | Manejar login, rol, menú visible y permisos de acción. |
...
| `email_contacto` | `VARCHAR(100)` | No | Email de contacto no necesariamente usado para login. |
...
## 10.3 Borrado lógico recomendado

Actualmente varias relaciones usan `ON DELETE CASCADE`. Esto simplifica el desarrollo, pero en un sistema tributario real puede ser peligroso porque una eliminación de persona o contribuyente puede eliminar comprobantes históricos.

Para producción se recomienda:

- evitar borrar contribuyentes con comprobantes;
- usar `activo`, `estado` o `deleted_at`;
- conservar comprobantes por auditoría;
- registrar usuario y fecha de eliminación lógica.
